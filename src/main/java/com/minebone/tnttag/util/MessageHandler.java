...
public class MessageHandler {
	...
	private static String getConfigMessage(String string) {
		if (Messages.getMessages().getString(string) != null) {
			return Messages.getMessages().getString(string).replace("&", "�")
					.replace("%newline", "\n");
		}
		Bukkit.getServer()
				.getLogger()
				.log(Level.SEVERE,
						"{0} Could not be found! If you have edited the messages.yml check that string. If nor, contact the plugin administrator.",
						new Object[] { string });
		return null;
	}
}
